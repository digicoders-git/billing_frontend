...
const ViewQuotation = () => {
    ...
    const handlePrint = useReactToPrint({
        ...
        documentTitle: quotation ? `Quotation_${quotation.quotationNo}` : 'Quotation'
    });
             ...
             <div ref={componentRef} className="bg-white shadow-lg rounded-xl overflow-hidden print:shadow-none print:rounded-none print:overflow-visible print:w-full">
                 ...
                 <div className="p-6 sm:p-8 border-b border-gray-200 flex flex-col sm:flex-row justify-between gap-4">
                    ...
                    <div className="text-left sm:text-right w-full sm:w-auto">
                        <h1 className="text-xl sm:text-2xl font-black text-gray-900 uppercase tracking-tight mb-1">Quotation</h1>
                        <p className="text-xs text-gray-500 font-bold mb-4">Original Copy</p>
                        <h2 className="text-lg font-bold text-gray-900">#{quotation.quotationNo}</h2>
                        <p className="text-xs text-gray-500 mt-1">Date: {new Date(quotation.date).toLocaleDateString('en-GB')}</p>
                        <p className="text-xs text-gray-500">Valid Until: {new Date(quotation.validityDate).toLocaleDateString('en-GB')}</p>
                    </div>
                 </div>
                 ...
                 <div className="p-6 sm:p-8 grid grid-cols-1 sm:grid-cols-2 gap-8 sm:gap-12">
                     <div>
                         <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">Estimate For</h3>
                         <div className="space-y-1">
                             <p className="text-sm font-bold text-gray-900">{quotation.partyName}</p>
                             {quotation.party && (
                                <>
                                    <p className="text-xs text-gray-500 max-w-full sm:max-w-[200px]">{quotation.party.billingAddress || 'No Address'}</p>
                                    <p className="text-xs text-gray-500 mt-2">Mobile: {quotation.party.mobile}</p>
                                </>
                             )}
                         </div>
                     </div>
                         ...
                         <div className="w-full sm:w-64 space-y-3">
                             ...
                             {quotation.additionalCharges > 0 && (
                                <div className="flex justify-between text-xs text-gray-500 font-medium">
                                    ...
                                    <span>+ ₹ {quotation.additionalCharges}</span>
                                </div>
                             )}
                             {quotation.overallDiscount > 0 && (
                                <div className="flex justify-between text-xs text-green-600 font-medium">
                                    <span>Discount</span>
                                    <span>- ₹ {quotation.overallDiscountType === 'percentage' 
                                        ? `${(quotation.subtotal * quotation.overallDiscount / 100).toFixed(2)} (${quotation.overallDiscount}%)` 
                                        : quotation.overallDiscount}
                                    </span>
                                </div>
                             )}
                             <div className="border-t border-gray-200 pt-3 flex justify-between items-center">
                                 <span className="text-sm font-black text-gray-900 uppercase">Total Amount</span>
                                 <span className="text-xl font-black text-gray-900">₹ {quotation.totalAmount}</span>
                             </div>
                         </div>
                     </div>
                 </div>
                 ...
                 <div className="p-6 sm:p-8 border-t border-gray-200 grid grid-cols-1 sm:grid-cols-2 gap-8 text-[10px] text-gray-500 break-inside-avoid">
                     <div className="space-y-2">
                         <h4 className="font-black text-gray-900 uppercase tracking-widest">Terms & Conditions</h4>
