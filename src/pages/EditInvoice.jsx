  ...
  useEffect(() => {
    const fetchInvoice = async () => {
      try {
        ...
        let partyId = (typeof invoice.party === 'object' && invoice.party !== null) ? invoice.party._id : invoice.party;
        let partyObj = parties.find(p => p._id === partyId);
        
        // Fallback: If not found in list, check if invoice.party is the object itself or construct from metadata
        if (!partyObj) {
            if (typeof invoice.party === 'object' && invoice.party !== null) {
                partyObj = invoice.party;
            } else if (invoice.partyName) {
                 partyObj = {
                    _id: partyId,
                    name: invoice.partyName,
                    billingAddress: invoice.billingAddress,
                    gstin: 'N/A', // Potentially missing in simple metadata
                    mobile: 'N/A',
                    type: 'Customer'
                 };
            }
